...
'''
WGAN-CP solves some of the stability issues with the GANs.
you'll use a special kind of loss function known as the W-loss and gradient penalties to prevent mode collapse.

Since the changes for WGAN-GP are done to the loss function during training, 
you can simply reuse your previous GAN code for the generator and critic class. 
Remember that in WGAN-GP, you no longer use a discriminator that classifies fake and real as 0 and 1 
but rather a critic that scores images with real numbers.
'''
...
# Initialize generator, citric and optimizers
...
'''
First, you calculate the magnitude of each image's gradient.
The magnitude of a gradient is also called the norm. 
Then, you calculate the penalty by squaring the distance between each magnitude and 
the ideal norm of 1 and taking the mean of all the squared distances.
'''
...
The argument has the scores for all fake images in the batch, but you will use the mean of them.
'''
